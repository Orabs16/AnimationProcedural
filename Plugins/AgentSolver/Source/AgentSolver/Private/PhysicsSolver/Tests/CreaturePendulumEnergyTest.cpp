...
namespace CreaturePendulumEnergyTest
{
	...
	static FCreatureTopology BuildPendulumTopology()
	{
		...
		// mode — so a near-fixed anchor is approximated two ways at once:
		//
		//   1. Overwhelming mass/inertia, so the rod's reaction barely moves it
		//      (checked below via MaxAnchorDisplacement, not just assumed).
		//   2. BodyGravityScale = 0, so gravity does not act on it at all.
		//
		// The second is NOT redundant with the first, and mass cannot substitute
		// for it: gravity accelerates every mass equally regardless of size, so a
		// 1e5 kg anchor free-falls at exactly -980 like everything else, and an
		// anchor+rod assembly falling together develops no internal stress — the
		// pendulum would simply not swing. Until 2026-08-21 this was done by
		// staging a constant, never-cleared counter-force through the external-
		// wrench path; that path is gone, and a per-body gravity scale expresses
		// the same intent as static topology data instead of as a force the
		// caller has to remember not to clear.
		Topo.BodyMass[0] = 1.0e5f;
		Topo.BodyInertiaDiagLocal[0] = FVector(1.0e7f, 1.0e7f, 1.0e7f);
		Topo.BodyGravityScale[0] = 0.0f;
