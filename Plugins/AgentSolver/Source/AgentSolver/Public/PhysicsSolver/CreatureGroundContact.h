...
// Ground contact for AgentSolver's ABA creature. Contact is resolved at the
// VELOCITY level, as impulses applied through the solver's own articulated
// response (see ResolveGroundContactImpulses) -- the penalty spring-damper this
// file originally described was removed 2026-08-14, and the external-wrench
// path it staged forces through was removed 2026-08-21 once nothing but tests
// still wrote to it.
//
...
#include "PhysicsSolver/CreatureBatchSolver.h" // velocity-level contact needs the impulse machinery
...
namespace CreatureGroundContact
{
	 ...
	 *
	 * Resolves each body's authored mass entry through Topo.BodyBoneIndex when
	 * the topology carries one, falling back to BodyDebugNames
	 * (MutoTopology::BuildMutoTopology's optional OutBodyDebugNames out-param)
	 * for topologies built without a reference skeleton. Per-point LimbIndex comes straight from
		...
		float StructuralRadiusFallback = 10.0f)
	{
		...
		for (int32 Body = 1; Body < Topo.NumBodies; ++Body)
		{
			...
			// Bone index first (see FCreatureTopology::BodyBoneIndex), name only as a
			// fallback: a topology not built from a reference skeleton -- the extracted
			// sub-chains MutoIsolatedLimbTest builds, which copy names but not indices --
			// leaves BodyBoneIndex at INDEX_NONE, and those still need to resolve.
			const int32 BoneIdx = Topo.BodyBoneIndex.IsValidIndex(Body) ? Topo.BodyBoneIndex[Body] : INDEX_NONE;
			const int32 MassIdx = (BoneIdx != INDEX_NONE)
				? MassAsset->FindBoneByIndex(BoneIdx)
				: MassAsset->FindBoneByName(BodyDebugNames[Body]);
